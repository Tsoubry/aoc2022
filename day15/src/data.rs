...
fn text_to_coordinate(text: &str) -> Coordinate {
    ...
    let x = captures.get(1).expect("first capture group").as_str().parse::<isize>().expect("parse to isize");
    let y = captures.get(2).expect("second capture group").as_str().parse::<isize>().expect("parse to isize");
