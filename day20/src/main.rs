...
fn find_xth_number(list: &VecDeque<isize>, n: usize) -> isize {
    ...
    let zero_index = list.iter().enumerate().find(|x| x.1 == &0).expect("no zero in list").0;
...
fn answer_part1(data: Vec<isize>) -> isize {
    ...
    for i in 0..list_size {
        ...
        let (index, _) = list.iter().enumerate().find(|item| item.1.0 == i).expect("Item with index not found").clone();

        let item = &mut list.remove(index).expect("item couldn't be removed from list");
        ...
        let final_index = match new_index.signum() {
            -1 => {
                list_size as isize + modulo - 1
            },
            0 => {
                list_size as isize - 1
            }
            _ => {  // positive
    ...
    find_xth_number(&item_only, 1000) + find_xth_number(&item_only, 2000) + find_xth_number(&item_only, 3000)
